@php
    ...
    $miniLogoText = $obj->miniLogoConnection(); //Get Mini Logo using Text
    // $unread = $obj->unreadMessage(); //Get unread Messages
    // $leaveMessages = $obj->noticeReadMessage(); //Get Notice Message
    $unread = 10; //Get unread Messages
@endphp
<!-- menu  -->
                    ...
                    <div class="header_notification_warp d-flex align-items-center">
                        <li>
                            <a class="CHATBOX_open nav-link-notify" href="#">
                                <img src="{{ asset('backend/assets/img/icon/msg.svg') }}" alt = "" />
                                @if ($unread > 0)
                                    <span>{{ $unread }}</span>
                                @endif
